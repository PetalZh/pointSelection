...
class NuScenesMetric(BaseMetric):
    ...
    def nus_evaluate(self,
        ...
        self.write_to_file(metric_dict, constants.sample_method, constants.sample_rate, constants.model_name, constants.feature_model)
        self.write_dict_data(metric_dict)
        return metric_dict

    def write_dict_data(self, metric_dict):
        path = '/home/xiaoyu/experiments/mmdetection3d/tools/output/nus_{}_{}_{}_{}.txt'.format(constants.sample_method, constants.sample_rate, constants.model_name, constants.feature_model)
        if not os.path.exists(path):
                f = open(path, "x")
        json.dump(metric_dict, open(path, 'w'))

    def write_to_file(self, result_dict, sample_method = '', sample_rate = '', model_name = '', feature_model = '', time = ''):
        file_name = 'result_nus.xlsx'
        if not os.path.isfile(file_name):
            wb = Workbook()
        else:
            wb = load_workbook('result_nus.xlsx')
        ...
        sheet.cell(row=1, column=column_index_from_string('E')).value = 'Bus'

        sheet.cell(row=1, column=column_index_from_string('F')).value = 'Trailer'
        sheet.cell(row=1, column=column_index_from_string('G')).value = 'Construction_vehicle'
        sheet.cell(row=1, column=column_index_from_string('H')).value = 'Pedestrian'
        ...
        sheet.cell(row=1, column=column_index_from_string('K')).value = 'Traffic_cone'
        sheet.cell(row=1, column=column_index_from_string('L')).value = 'Barrier'

        sheet.cell(row=1, column=column_index_from_string('M')).value = 'Overall'
        sheet.cell(row=1, column=column_index_from_string('N')).value = 'Time'

        # sheet.cell(row=1, column=column_index_from_string('O')).value = 'Time'
        # sheet.cell(row=1, column=column_index_from_string('P')).value = 'IR_Car'
        # sheet.cell(row=1, column=column_index_from_string('Q')).value = 'IR_Pedestrian'
        # sheet.cell(row=1, column=column_index_from_string('R')).value = 'IR_Cyclist'


        row = sheet.max_row +1
        print('row_number: ', row)
        sheet.cell(row=row, column=column_index_from_string('A')).value = sample_method + ' ' + feature_model
        sheet.cell(row=row, column=column_index_from_string('B')).value = sample_rate

        sheet.cell(row=row, column=column_index_from_string('C')).value = result_dict['pred_instances_3d_NuScenes/car_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('D')).value = result_dict['pred_instances_3d_NuScenes/truck_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('E')).value = result_dict['pred_instances_3d_NuScenes/bus_AP_dist_1.0']

        sheet.cell(row=row, column=column_index_from_string('F')).value = result_dict['pred_instances_3d_NuScenes/trailer_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('G')).value = result_dict['pred_instances_3d_NuScenes/construction_vehicle_AP_dist_2.0']
        sheet.cell(row=row, column=column_index_from_string('H')).value = result_dict['pred_instances_3d_NuScenes/pedestrian_AP_dist_1.0']

        sheet.cell(row=row, column=column_index_from_string('I')).value = result_dict['pred_instances_3d_NuScenes/motorcycle_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('J')).value = result_dict['pred_instances_3d_NuScenes/bicycle_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('K')).value = result_dict['pred_instances_3d_NuScenes/traffic_cone_AP_dist_1.0']
        sheet.cell(row=row, column=column_index_from_string('L')).value = result_dict['pred_instances_3d_NuScenes/barrier_AP_dist_1.0']

        sheet.cell(row=row, column=column_index_from_string('M')).value = result_dict['pred_instances_3d_NuScenes/mAP']
        sheet.cell(row=row, column=column_index_from_string('N')).value = ''
    ...
    def _evaluate_single(
        ...
        print('evaluation version: ', self.version)
