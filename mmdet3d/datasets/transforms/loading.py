...
class LoadPointsFromMultiSweeps(BaseTransform):
    ...
    def __init__(self,
        ...
        self.octree_height = octree_height
    ...
    def transform(self, results: dict) -> dict:
        ...
        # # np.savetxt('pc_before_0.1{}.csv'.format(constants.counter), points, delimiter=",")
        # points = doSample(points, self.sample_method, self.sample_rate, self.oc_node_capacity, self.octree_height)
        # # np.savetxt('pc_after_0.1{}.csv'.format(constants.counter), points, delimiter=",")

        # points = LiDARPoints(torch.from_numpy(points), 4)
...
class LoadPointsFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # constants.file_names.append(self.pts_filename)
        # instance_points_before = self.getInstancePoints(self.pts_filename, points)
        
        time_start = time.time()
        print(len(points))
        points = doSample(points, self.sample_method, self.sample_rate, self.pts_filename, self.oc_node_capacity, self.octree_height)
        time_end = time.time()
        ...
        # if constants.counter == 3769:
        #     path = '/home/xiaoyu/experiments/mmdetection3d/tools/pc_list.json'
        #     if not os.path.exists(path):
        #         f = open(path, "x")
        #     json.dump(constants.file_names, open(path, 'w'))
    ...
    def getSimilarities(self, leaves):
        print('start one point cloud: ')
        for cluster in leaves:
            similarity_matrix = cosine_similarity(cluster[:, 4:]) #euclidean_distances(points[:, 4:])
            overall_similarity = similarity_matrix[np.triu_indices(similarity_matrix.shape[0], k=1)].mean()

            print('cosine sim: {}, cluster size: {}'.format(overall_similarity, len(cluster)))
    ...
    def getStatistics(self, leaves):
        ...
        slot_10 = 0
        slot_0 = 0
        print('Total number of leaves: ', len(leaves))
        for item in leaves:
            if len(item) >= 500:
                slot_500 += 1
            if 300 <= len(item) < 500:
            ...
            if len(item) < 10:
                slot_0 += 1
        print('>=500: {}, 300-500: {}, 200-300: {}, 100-200: {}, 50-100: {}'.format(slot_500, slot_300, slot_200, slot_100, slot_50))
        print('40-50: {}, 30-40: {}, 20-30: {}, 10-20: {}, <10: {}'.format(slot_40, slot_30, slot_20, slot_10, slot_0))
