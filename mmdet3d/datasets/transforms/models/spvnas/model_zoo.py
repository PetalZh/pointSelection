...
def spvnas(net_id, pretrained=True):
    path = '/home/xiaoyu/experiments/mmdetection3d/checkpoints/spvnas/net.config'
    conf_file = open(path)
    net_config = json.load(conf_file)
    ...
    model = SPVNAS(
        ...
        vres=net_config['vres']).to(device)
    model.manual_select(net_config)
    model = model.determinize()

    model_file = '/home/xiaoyu/experiments/mmdetection3d/checkpoints/spvnas/spvnas.pth'
    if pretrained:
        init = torch.load(model_file)['model']
        model.load_state_dict(init)
    return model

def spvcnn(net_id, pretrained=True):
    path = '/home/xiaoyu/experiments/mmdetection3d/checkpoints/spvcnn/net.config'
    conf_file = open(path)
    net_config = json.load(conf_file)
    device = torch.device("cuda:0") #"cuda:0" if torch.cuda.is_available() else 
    ...
    model_file = '/home/xiaoyu/experiments/mmdetection3d/checkpoints/spvcnn/spvcnn.pth'
    if pretrained:
        init = torch.load(model_file)['model']
        model.load_state_dict(init)
    return model
